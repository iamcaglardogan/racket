...
Phase 3 adds a headless `RemovalEngine`, `ManifestStore`, and `UndoService`. Verification and the owner's checkpoint review are pending. No cleanup rule or product interface is enabled. Tests use synthetic homes and an injected fake Trash transport; they do not touch the user's home or real Trash.
...
Apple's Trash API accepts a pathname and provides the resulting location through its output parameter. RACKET records that returned path rather than constructing a name in Trash. [Apple Foundation Trash API](https://developer.apple.com/documentation/foundation/filemanager/trashitem(at:resultingitemurl:))
...
The [test plan](TESTING.md) distinguishes real Darwin moves and journal files inside synthetic `/private/tmp` homes from the injected fake Trash transport. Actual Foundation Trash movement and recognition, Finder Put Back, genuine cloud placeholders, separate-volume behavior, power-loss durability, and macOS 14 runtime compatibility remain unverified. No user home, real Trash, application cache, or project is used as test data.
