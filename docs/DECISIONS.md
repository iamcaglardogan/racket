...
Bundled rule-set version `1.1.0` contains one Camera Raw candidate with `verified: false` and `enabled: false`. Its provisional producer list includes Lightroom Classic, Photoshop, Bridge, After Effects, and the After Effects Render Engine; it is not a complete host catalogue. Bridge's executable identity remains unverified, so the declared set currently yields unknown activity. Compiled safe roots remain unchanged. Metadata observations and a read-only native-settings inspection on September 17 confirm only configured parents: Resolve’s first Media Storage location is `~/Movies` and proxy generation follows the project setting; After Effects shows `~/Library/Caches` for disk cache and `~/Library/Application Support/Adobe/Common/` for shared media cache/database. These mixed parents do not establish cache-specific scope or cleanup safety. Dedicated Resolve candidates were refused or missing, and bounded enumeration found no direct names containing “after effects” or “disk cache”. No preferences or permissions were changed. [CREATIVE-CACHE.md](CREATIVE-CACHE.md) records the evidence, deferred locations, and remaining checks.
