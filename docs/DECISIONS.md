...
Bundled rule-set version `1.1.0` contains one Camera Raw candidate with `verified: false` and `enabled: false`. Its provisional producer list includes Lightroom Classic, Photoshop, Bridge, After Effects, and the After Effects Render Engine; it is not a complete host catalogue. Bridge's executable identity remains unverified, so the declared set currently yields unknown activity. Compiled safe roots remain unchanged. Metadata observations on September 13 and 17 and the owner's confirmation that Resolve and After Effects use default locations inform the candidate review; they do not verify configured paths, contents, or cleanup safety. [CREATIVE-CACHE.md](CREATIVE-CACHE.md) records the evidence, deferred locations, and remaining checks.
