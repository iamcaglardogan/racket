...
`DirectoryWalker` enumerates each rule's `directoryContents` scope. It validates every child against protected-path rules and emits ordinary regular files only. It does not emit directories, approve their unexamined contents, infer project ownership, or choose which files to remove. A finding carries its resolved path, allocated size, modification date, rule ID, module, risk, reason, and regeneration note. Phase 3 also retains the live walk's internal metadata fingerprint for later comparison. Publicly constructed synthetic findings lack that observation and cannot authorize removal. `judgement` findings are ineligible for preselection; eligibility for other risks is not user approval.
...
These checks reduce the chance of reporting a substituted path. They are not an atomic snapshot of a changing filesystem. Foundation's pathname queries remain observations, and a later consumer cannot treat a finding as permission to move a file. Phase 3 independently rechecks rule scope, the retained scan fingerprint, allocated size, and live paths before capture; its final Foundation Trash call retains a documented pathname race.
